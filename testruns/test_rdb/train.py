...
opt['batch_size'] =8 
opt['atWhichSave'] = [2,100000,150000,200000,250000,300000,350000,400000,450000,500000,550000, 600000,650002,700002,750000,800000,850002,900002,950000,1000000] # testing will be done at these iterations and corresponding model weights will be saved.
opt['iterations'] = 500000 # The model will run for these many iterations.
...
train_files = glob.glob('/home/demibreen/project/data/Sony/short/short_train_val/*.ARW')
gt_files =glob.glob('/home/demibreen/project/data/Sony/long/long_train_val/*.ARW')
# If you have less CPU RAM you would like to use fewer images for training.
if dry_run:
    train_files = train_files[:2]
    opt['iterations'] = dry_run_iteration

    
dataloader_train = DataLoader(load_data(train_files,gt_files,train_amplification_file,20,gt_amp=False,training=True), batch_size=opt['batch_size'], shuffle=True, num_workers=0, pin_memory=True)
...
test_files = glob.glob('/home/demibreen/project/data/Sony/short/short_test/*.ARW') 
gt_files +=glob.glob('/home/demibreen/project/data/Sony/long/long_test/*.ARW')
if dry_run:
    test_files = test_files[:2]
    
dataloader_test = DataLoader(load_data(test_files,gt_files,test_amplification_file,2,gt_amp=False,training=False), batch_size=1, shuffle=False, num_workers=0, pin_memory=True)
